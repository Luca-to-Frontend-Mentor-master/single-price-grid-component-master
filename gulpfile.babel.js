...
// Configuración  para compilar  CSS
/**
 * gulp.task('styles', () => {
	return gulp
		.src('./src/css/*.css') // Carpetas que utilizamos
		.pipe(concat('styles-min.css')) //Une todos nuestros archivos js en uno solo
...
gulp.task('clean', () => {
	return gulp
		.src('./public/css/styles.css') // Carpetas que utilizamos
		.pipe(
			clean({
				content: ['./public/*.html'],
			}),
